...
logging.basicConfig(
        filename='./log/log.log',filemode='a',\
...
#문서수집시에 사용할 프로세스의 개수
CRAWLER_PROCESS_QUANTITY=4

#url서버에 후보 url을 요청할 때, 한번 요청시의 url 개수
URL_REQUEST_QUANTITY=500

#url서버의 포트번호
URL_SERVER_PORT=2224

#url서버의 IP주소
URL_SERVER_IP='192.168.0.4'
    ...
    def _apply_config(self, config_file):
        ...
        for each_line in config_file.xreadlines():
            ...
            if argument=='CATEGORY':
                self.CATEGORY=value
            elif argument=='CRAWLER_PROCESS_QUANTITY':
                self.CRAWLER_PROCESS_QUANTITY=int(value)
            elif argument=='URL_REQUEST_QUANTITY':
                self.URL_REQUEST_QUANTITY=int(value)
            elif argument=='URL_SERVER_PORT':
                self.URL_SERVER_PORT=int(value)
            elif argument=='URL_SERVER_IP':
                self.URL_SERVER_IP=value
